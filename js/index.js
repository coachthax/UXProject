...
function makeNetworkRequest(query) {
  return new Promise(function(resolve, reject) {
    var API_URL =
      "https://api.spotify.com/v1/search?limit=3&type=artist,track,album,playlist&q=" +
      encodeURI(query);
...
function listItem() {
  this.render = function() {
    ...
    img.src =
      this.src === 0
        ? "https://samratcliffe.github.io/images/placeholder.jpg"
        : this.src;
...
function resultListItem(resultItem) {
  this.title =
    ...
    resultItem.external_urls.spotify +
    "'>" +
    (resultItem.name.length > 45
      ? resultItem.name.substr(0, 45) + "…"
      : resultItem.name) +
    "</a>";
  ...
  switch (resultItem.type) {
    case "artist":
      this.subtitle =
        Number(resultItem.followers.total).toLocaleString() + " listeners";
    ...
    case "track":
      this.subtitle =
        "<a target='_blank'  href='" +
        resultItem.artists[0].external_urls.spotify +
        "'>" +
        resultItem.artists[0].name +
        "</a>" +
        " • " +
        "<a target='_blank' href='" +
        resultItem.album.external_urls.spotify +
        "'>" +
        resultItem.album.name +
        "</a>";
      this.src =
        resultItem.album.images.length &&
        resultItem.album.images.slice(-1)[0].url;
      break;
    case "playlist":
